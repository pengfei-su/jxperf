...
__thread WatchpointStack_t wpStack ={.top = -1};
...
static bool ValidateWPData(SampleData_t *sampleData) {
    ...
    switch (sampleData->watchLen) {
        case 0: EMSG("\nValidateWPData: 0 length WP never allowed"); return false;
        case 1:
        case 2:
        case 4:
        case 8:
            if(IS_ALIGNED(sampleData->va, sampleData->watchLen))
                return true; // unaligned
            else
                return false;
            break;
        default:
            EMSG("Unsuppported WP length %d", sampleData->watchLen);
...
static bool IsOverlapped(SampleData_t *sampleData) {
    WP_ThreadData_t * tData = TD_GET();
    // Is a WP with the same/overlapping address active?
    for (int i = 0; i < wpConfig.maxWP; i++) {
        if(tData->watchPointArray[i].isActive) {
            if(ADDRESSES_OVERLAP((uint64_t)tData->watchPointArray[i].sd.va, tData->watchPointArray[i].sd.watchLen, (uint64_t)sampleData->va, sampleData->watchLen)){
                return true;
            }
        }
    }
...
static inline void CaptureValue(SampleData_t * sampleData, void *valLoc) {
    switch(sampleData->watchLen) {
...
static void CreateWatchPoint(WP_RegisterInfo_t *wpi, SampleData_t * sampleData, bool modify) {
    // Perf event settings
...
static bool ArmWatchPoint(WP_RegisterInfo_t *wpi, SampleData_t * sampleData) {
...
bool WP_Subscribe(SampleData_t *sampleData, bool isCaptureValue, bool isVarianceClient) {
    ...
    if(r != WP_VICTIM_NONE_AVAILABLE) {
        if (isCaptureValue) {
            CaptureValue(sampleData, &(tData->watchPointArray[victimLocation].sampleValue[0]));
        }
    ...
    *numAddr = 0;
    for (int idx = 0; idx < wpConfig.maxWP; idx++) { 
        if (tData->watchPointArray[idx].isActive) {
           addrs[(*numAddr)++] = tData->watchPointArray[idx].sd.va;  
        } 
    } 
}
...
void WP_GetActiveWatchPoints(WP_TriggerInfo_t wpt[], int *numActiveWP) {
    ...
    *numActiveWP = 0;
    for (int idx = 0; idx < wpConfig.maxWP; idx++) { 
        ...
        if (wpi->isActive) wpt[(*numActiveWP)++].sd = &(wpi->sd); 
    } 
}
